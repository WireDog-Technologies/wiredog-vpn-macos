 ...
 *   "PENDING"          – needs user approval in System Settings → Privacy & Security
...
struct TSFNHolder {
    ...
    void invoke(const std::string& result) {
        // Guard against multiple invocations (requestNeedsUserApproval may fire
        // before didFinishWithResult on some macOS versions)
        bool expected = false;
        if (!called.compare_exchange_strong(expected, true)) return;
...
- (void)request:(OSSystemExtensionRequest *)request
    ...
    if (result == OSSystemExtensionRequestWillCompleteAfterReboot) {
        _holder->invoke("REBOOT_REQUIRED");
    } else {
        _holder->invoke("ACTIVATED");
    }
}
