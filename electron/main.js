...
// ================================
const { BrowserWindow, ipcMain, Menu, Tray, shell, dialog, nativeImage, safeStorage } = require('electron');
...
app.whenReady().then(async () => {
  ...
  // Show a native dialog when the system extension needs one-time user approval.
  // macOS Security requires this for all Network Extensions distributed via Developer ID.
  // The dialog appears once at first install and guides the user to System Settings.
  vpnService.onExtensionNeedsApproval(() => {
    log.info('VPN: Extension needs user approval — showing System Settings dialog');
    dialog.showMessageBox({
      type: 'information',
      title: 'One-Time VPN Setup Required',
      message: 'Allow WireDog VPN Network Extension',
      detail: 'macOS requires a one-time approval to enable the VPN tunnel.\n\nIn System Settings, go to Privacy & Security and click "Allow" next to WireDog VPN. You only need to do this once.',
      buttons: ['Open System Settings', 'Later'],
      defaultId: 0,
      cancelId: 1,
    }).then(({ response }) => {
      if (response === 0) {
        shell.openExternal('x-apple.systempreferences:com.apple.preference.security?Privacy_NetworkExtensions');
      }
    });
  });
