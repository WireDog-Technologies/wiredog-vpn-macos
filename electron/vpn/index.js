...
class VPNService {
  ...
  /**
   * Register a callback to be fired when the system extension needs user approval.
   * Called once, when OSSystemExtensionManager returns the PENDING state
   * (activated_waiting_for_user in sysextd). Main process uses this to show a
   * native dialog guiding the user to System Settings → Privacy & Security.
   */
  ...
  async connect(serverId, settings, token = '') {
    ...
    try {
      ...
      // Store session info
      // publicIp = the resolved IPv4 of the VPN server endpoint.
      // This is what external services see when the tunnel is active.
      this.currentSession = {
        sessionId,
        server,
        assignedIp: config.address,
        publicIp: resolvedServerIp || server?.ipAddress || '',
