...
# Regenerate HelperVersion from package.json's "version" field, so the helper daemon can
# never report a stale version again (this is the durable fix for the class of bug iOS hit
# with its tunnel extension's bundle version drifting from the app's — see main.swift and
# JsonRpcHandler.swift's ping handler, which both reference HelperVersion.current).
APP_VERSION=$(node -p "require('$PROJECT_ROOT/package.json').version")
echo "=== Generating HelperVersion ($APP_VERSION) ==="
cat > Sources/GeneratedVersion.swift <<EOF
// GENERATED FILE — do not edit by hand, do not commit.
// Regenerated by scripts/build-helper.sh from the root package.json's "version" field
// before every build.
enum HelperVersion {
    static let current = "$APP_VERSION"
}
EOF
