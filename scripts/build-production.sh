...
if [ "$VERBOSE" = "1" ]; then
  ...
  npm run package:mac 2>&1 | tee /tmp/wiredog-package.log | grep -E "(signing|notariz|afterPack|ERROR|error|building)" || true
...
if [ "$NOTARIZE" = "1" ]; then
  step "Verifying notarization with spctl"
  if spctl -a -vvv -t install "$APP_PATH" 2>&1 | tee /tmp/wiredog-spctl.log | grep -q "accepted"; then
    ok "Notarization accepted by Gatekeeper"
  else
    cat /tmp/wiredog-spctl.log
    warn "spctl did not report 'accepted' — notarization may still be in progress.
     Retry in a few minutes: spctl -a -vvv -t install \"$APP_PATH\""
  fi
fi
