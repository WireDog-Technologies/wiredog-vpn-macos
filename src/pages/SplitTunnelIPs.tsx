...
function isValidIpOrCidr(value: string): boolean {
  ...
  // IPv4 with optional CIDR — no leading zeros allowed (e.g. 192.168.001.0 is invalid)
  ...
  if (ipv4.test(trimmed)) {
    ...
    // Reject leading zeros in any octet (e.g. "01", "001")
    if (parts.some(p => p.length > 1 && p.startsWith('0'))) return false;
    if (parts.map(Number).some(p => p > 255)) return false;
...
const SplitTunnelIPs: React.FC = () => {
  ...
  const addIp = () => {
    ...
    if (!isValidIpOrCidr(trimmed)) {
      setError('Invalid IP address or CIDR notation. Leading zeros are not allowed (e.g. use 192.168.1.0 not 192.168.01.0)');
      return;
    }
