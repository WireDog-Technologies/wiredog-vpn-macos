...
import { ConnectionInfo, ConnectionStatus, VPNSettings, ServerLocation, User, Protocol } from '@/types/vpn';
...
export const VPNProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    //
    // Always-On Kill Switch caveat: when persistent_block pf rules are active,
    // /auth/me is unreachable. We detect that and hydrate from the cached user
    // profile so the UI can render past the login screen (otherwise the user
    // is locked out until they run Emergency Reset). Once the tunnel comes
    // back up, a background refresh reconciles the cached profile.
    const checkAuth = async () => {
      try {
        let killSwitchBlocking = false;
        try {
          const vpnStatus = await window.electronAPI?.vpn?.getStatus?.();
          killSwitchBlocking =
            !!vpnStatus?.advancedKillSwitchActive && vpnStatus?.status !== 'connected';
        } catch {
          // getStatus can fail before the helper pipe is ready — treat as not blocking
        }

        if (killSwitchBlocking) {
          const cached = await window.electronAPI?.auth?.getUser?.();
          if (cached) {
            setUser(cached as User);
            // Fire-and-forget refresh once the tunnel is up
            api.getCurrentUser().then(me => {
              if (me) {
                const mapped = mapMeResponseToUser(me);
                setUser(mapped);
                window.electronAPI?.auth?.setUser?.(mapped);
              }
            }).catch(() => { /* ignored */ });
            return;
          }
          // No cached profile — fall through so the login UI renders and the
          // user can use Emergency Reset from the login screen.
          return;
        }
        ...
        if (window.electronAPI?.vpn?.getStatus) {
          ...
          if (status?.status === 'connected' && status.session) {
            ...
            setConnection({
              ...
              ipAddress: status.session.publicIp || status.session.server?.ipAddress || status.session.assignedIp || '',
  ...
  // invalid) — mirrors iOS's AuthService.handleUnauthorized(). Clears local session state
  // only; there's no point calling the authenticated /auth/logout endpoint with a token
  // the backend has already rejected. Clearing `user` routes back to the login screen via
  // the existing isAuthenticated-gated routing in App.tsx.
    ...
    try {
      ...
      if (window.electronAPI?.vpn) {
        ...
        setConnection({
          status: 'connected',
          server,
          ipAddress: result.publicIp || result.server?.ipAddress || result.assignedIp,
      ...
      if (message === 'WIREDOG_UNAUTHORIZED') {
  ...
  // mirrors iOS's VPNManager.waitForDisconnected().
  ...
  // disconnect-old-then-connect-new sequence behind quick server switching. Mirrors iOS's
  // VPNManager.switchServer(to:).
  ...
  // highlight, same as before; the user still hits Connect for that one. Mirrors iOS's
  // VPNManager.selectServer().
  ...
  useEffect(() => {
    if (window.electronAPI?.vpn) {
      const unsubscribe = window.electronAPI.vpn.onStatusChange((status) => {
        // Note: Do NOT sync killSwitchEnabled from backend — that reflects runtime firewall state,
        ...
        if (status.status === 'connected' && status.session) {
          ...
          setConnection(prev => ({
            ...
            ipAddress: prev.ipAddress || status.session!.publicIp || status.session!.server?.ipAddress || status.session!.assignedIp || '',
