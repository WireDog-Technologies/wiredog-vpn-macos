...
const USMap: React.FC = () => {
  ...
  // Helper to safely log to main.log
  const logToMain = useCallback((level: 'debug' | 'info' | 'warn' | 'error', msg: string) => {
    try {
      const api = (window as any).electronAPI?.log;
      if (api && api[level]) {
        api[level](msg);
      } else {
        console.error(`[LogFail] ${level}: ${msg}`);
      }
    } catch (err) {
      console.error(`[LogFail] Could not log: ${msg}`, err);
    }
  }, []);
  ...
  useEffect(() => {
    if (!isConnected) {
      ...
      logToMain('debug', '[Speed] VPN disconnected - resetting speed data');
      setSpeedData({
        downloadSpeed: 0,
        uploadSpeed: 0,
        downloadHistory: [],
    ...
    logToMain('debug', '[Speed] VPN connected - starting speed monitoring');
      ...
      try {
        ...
        try {
          if (window.electronAPI?.vpn?.getStats) {
            stats = await window.electronAPI.vpn.getStats();
          } else {
            logToMain('error', '[Speed] getStats method not available on electronAPI');
          }
        } catch (err) {
          logToMain('warn', `[Speed] Failed to get stats from helper: ${err.message}`);
        }
        ...
        // Fallback: simulate speed data when helper stats unavailable
        ...
        setSpeedData(prev => {
          ...
          // Only log fallback start, not every poll
          if (newHistory.downloadHistory.length === 1) {
            logToMain('info', `[Speed] Using fallback (helper stats unavailable). Down: ${Math.round(downloadSpeed / 1024)} KB/s, Up: ${Math.round(uploadSpeed / 1024)} KB/s`);
          }
          return newHistory;
        });
      } catch (error) {
        logToMain('error', `[Speed] Speed monitoring error: ${error.message}`);
      }
    };

    // Poll every second
    const interval = setInterval(pollSpeed, 1000);
    logToMain('debug', '[Speed] Starting speed polling interval (1s)');
    pollSpeed(); // Initial poll

    return () => {
      logToMain('debug', '[Speed] Clearing speed polling interval');
      clearInterval(interval);
    };
  }, [isConnected, logToMain]);
