...
const ReconnectDialog: React.FC<ReconnectDialogProps> = ({ open, onReconnect, onDismiss, message }) => {
  if (!open) return null;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-card border border-border rounded-xl p-6 max-w-sm mx-4 shadow-lg">
        ...
        <p className="text-sm text-muted-foreground mb-4">
          {message ?? 'Settings have changed. Reconnect now to apply, or apply on next connection.'}
        </p>
