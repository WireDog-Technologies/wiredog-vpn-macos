...
import { Loader2, Shield, ShieldOff, Power, Split, ShieldCheck, Lock } from 'lucide-react';
        ...
        <div className="flex justify-center mb-4">
          <button
            ...
            className={cn(
              ...
              isConnecting && "border-connection-connecting bg-connection-connecting/10 glow-effect-gold",
          ...
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger asChild>
                <button
                  onClick={() => {
                    if (!settings.killSwitch && settings.splitTunneling?.enabled) return;
                    const newValue = !settings.killSwitch;
                    updateSettings({ killSwitch: newValue });
                    if (isConnected) {
                      setReconnectMessage(
                        newValue
                          ? 'Kill switch has been enabled. Reconnect now for a clean protected session, or it will apply on your next connection.'
                          : 'Kill switch has been disabled. Reconnect now to remove protection, or it will apply on your next connection.'
                      );
                      setShowReconnectDialog(true);
                    }
                  }}
                  disabled={isConnecting || settings.splitTunneling?.enabled}
                  className={cn(
                    "flex items-center justify-center gap-2 w-full py-3 rounded-lg border transition-all duration-200",
                    (isConnecting || settings.splitTunneling?.enabled) && "opacity-50 cursor-not-allowed",
                    settings.killSwitch
              ...
              <TooltipContent>
                <p>
                  {settings.splitTunneling?.enabled
                    ? 'Kill switch unavailable while split tunneling is active'
