...
const AppContent = () => {
  ...
  useEffect(() => {
    ...
    // Toggle this to true to "freeze" loading for testing
  const holdLoadingForTesting = false;

  useEffect(() => {
    if (!holdLoadingForTesting) {
  ...
  }, [holdLoadingForTesting, isCompactMode]);
  ...
  if (isLoading || holdLoadingForTesting) {
